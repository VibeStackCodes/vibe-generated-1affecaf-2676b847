...
Be the all-in-one spend-management platform for SMBs, automatically ingesting card charges, enabling quick category-based expense entry, and delivering a real-time analytics dashboard that reveals spend patterns over any date range—eliminating spreadsheets and improving budgeting and reporting.

- Card expense ingestion: ingest and display transactions from connected cards/bank feeds with per-transaction fields (id, date, amount, currency, merchant, category, card_id, receipt_url, is_reimbursable, notes); automatic categorization options with manual edit capability; map merchant/date/amount/currency rules and handle currency conversions where needed. 
- Manual expense entry by category: quick-add form with category picker supporting custom categories and subcategories; bulk CSV import with mapping; templates for recurring entries; validation rules (required fields, currency, date range); basic category taxonomy management (add, rename, archive).
- Analytics dashboard: category spend over time (line chart with date range picker); top spend categories (bar/pie); spend by card/merchant filters; period-over-period comparisons; export options (CSV/PNG); drill-down to transaction-level view; dashboard widgets, bookmarks, and sharing.
- Data quality and security: data validation, audit trail for edits, role-based access control (owners, admins, viewers); encryption at rest and TLS in transit; basic access logging.
- Integrations and data sources: integrations with card f
